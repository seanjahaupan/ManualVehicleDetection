...
def extract_all_features(globalVariable):
    ...
    t=time.time()
    globalVariable.car_features = extract_features(car_images, globalVariable.color_space, orient = globalVariable.orient, 
                        pix_per_cell = globalVariable.pix_per_cell, cell_per_block = globalVariable.cell_per_block, 
                        hog_channel = globalVariable.hog_channel)
    globalVariable.noncar_features = extract_features(noncar_images, color_space = globalVariable.color_space, orient = globalVariable.orient, 
                        pix_per_cell = globalVariable.pix_per_cell, cell_per_block = globalVariable.cell_per_block, 
                        hog_channel = globalVariable.hog_channel)
    t2 = time.time()
...
'''
carImg = cv2.imread(car_images[0])
...
noncarImg = cv2.imread(noncar_images[0])
...
visualizeImage('car img', carImg)
visualizeImage('car img dst', carImgDst)
visualizeImage('non car img', noncarImg)
visualizeImage('non car img dst', noncarImgDst)
'''
...
#Train model using HOG features
train_model(globalVariable)

#make sure we normalize the incoming image first
image = mpimg.imread('test4.jpg')

#Update the y start stop to skip looking at the sky
'''
windows = slide_window(image, x_start_stop = [None, None], y_start_stop = [None, None],
                       xy_window = (96, 96), xy_overlap = (0.5, 0.5))
                       '''
'''
hot_windows = search_windows(image, windows, globalVariable.svc, globalVariable.X_scaler, color_space= globalVariable.color_space, 
                        spatial_size=spatial_size, hist_bins=hist_bins, 
                        orient=orient, pix_per_cell=pix_per_cell, 
                        cell_per_block=cell_per_block, 
                        hog_channel=hog_channel, spatial_feat=spatial_feat, 
...
out_img = find_cars(image, ystart, ystop, scale, globalVariable)
